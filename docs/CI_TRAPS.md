...
| New env var added to `env.validation.ts` passes unit tests but fails the CI job that uses it with `Invalid input: expected string, received undefined` | The `env:` block in `pr-tests.yml` for that job does not include the new var — no `.env` file exists in the CI runner | Add the env var to the `env:` block of every CI job that loads the app (e.g. `bff-component`); CI integration tests get it from Testcontainers / process env |
