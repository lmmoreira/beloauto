...
- [ ] **Resend invite affordance** — UC-028 A2 reactivates an inactive record only as a side effect of re-submitting the invite form with the same email. Should the team list also expose a direct "Reenviar convite" button on a still-pending (never-activated) row, instead of requiring the admin to re-open the invite form and retype the email?
