...
- [ ] **Success state UX** — prototype `02-approve-success.html` is a separate page for review clarity, but production intent is an inline state on the detail page. Confirm: does the admin stay on the detail page after approval, or auto-navigate back to the queue?
- [ ] **Reject/info success** — after REJECTED or INFO_REQUESTED, does the admin stay on the detail page (inline banner) or auto-navigate back to the queue?
